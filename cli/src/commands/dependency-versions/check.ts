...
export default class DependencyVersionsCheck extends BaseCommand {
  static summary =
    "Checks for @player-ui/@player-lang dependency version mismatches and issues warnings/solutions accordingly";
  ...
  async run(): Promise<{
    ...
    if (
      ...
      if (!topLevelVersionsExist && nestedVersions.length === 1) {
        console.log(
          `No top-level @player-ui/@player-lang dependencies exist. Only a single nested @player-ui/@player-lang version exists, ${nestedVersions[0]}`,
        );
      }

      if (!nestedVersionsExist && topLevelVersions.length === 1) {
        console.log(
          `No nested @player-ui/@player-lang dependencies exist. Only a single top-level @player-ui/@player-lang version exists, ${topLevelVersions[0]}`,
        );
      }
    ...
    if (multipleTopLevelVersionsDetected) {
      ...
      if (cmp(highestTopLevelVersion, highestNestedVersion) >= 1) {
        ...
        console.log(
          `- The highest @player-ui/@player-lang version is ${highestTopLevelVersion} at the top level. Please add resolutions for all nested @player-ui/@player-lang versions to this version or bump the nested versions to it.`,
        );
      } else {
        console.log(
          `- The highest @player-ui/@player-lang version is ${highestNestedVersion} at the nested level. Please bump the top-level version, ${highestTopLevelVersion}, to ${highestNestedVersion}.`,
        );
        if (multipleNestedVersionsDetected) {
          console.log(
            `- Also, please add resolutions or bump the versions for nested @player-ui/@player-lang dependencies whose version is not ${highestNestedVersion}.`,
          );
        }
      }
      ...
      console.log(
        `- The highest @player-ui/@player-lang version is ${highestNestedVersion} at the nested level. Please add resolutions for all nested @player-ui/@player-lang versions to this version or bump the nested versions to it.`,
      );
    }
